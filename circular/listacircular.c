...
int inserirElemento(Lista *lista, int valor){
    ...
    if(lista->prim == NULL){
        nova->prox = nova;
        /*vai preparar a caixinha para receber o elemento na lista*/
        lista->prim = nova->prox;
        lista->prim = nova;
        return 1;
    }

    /*Procurando o ultimo ponteiro da lista para receber o novo elemento*/
    /*será criado um ponteiro p que irá percorrer pela lista*/
...
int removerElemento(Lista *lista, int valor){
    ...
    if( lista->prim->valor == valor){
        aux = lista->prim;
        for(p = lista->prim; p->prox != aux; p= p->prox);
        p->prox = aux->prox;
        lista->prim = lista->prim->prox;
        free(p);
        return 1;
